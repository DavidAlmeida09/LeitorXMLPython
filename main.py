...
lista_arquivos = os.listdir("NFs") # lista os nomes dos arquivos de uma pasta
...
for arquivo in lista_arquivos: # para cada arquivo
    if 'xml' in arquivo: # se tem xml no nome do arquivo
        if 'DANFE' in arquivo: # se tem DANFE no nome do arquivo
            print(ler_xml_danfe(f'NFs/{arquivo}')) # rodar o leitor de XML de DANFE para esse arquivo
        else:
            print(ler_xml_servico(f'NFs/{arquivo}'))
